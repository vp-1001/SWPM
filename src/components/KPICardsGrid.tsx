...
export const KPICardsGrid: React.FC<KPICardsGridProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-3.5">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[10px] text-emerald-400 font-medium">{kpis.overallWQI.statusLabel}</div>
            </div>
          </div>
          {kpis.overallWQI.isSimulated && (
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[10px] text-emerald-400 font-medium">{kpis.pH.statusLabel}</div>
            </div>
          </div>
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[10px] text-blue-400 font-medium">{kpis.tds.statusLabel}</div>
            </div>
          </div>
          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-mono text-slate-400 bg-slate-800/80">
            PPM
      ...
      {/* 4. Turbidity */}
      {(() => {
        const isWarn = kpis.turbidity.status === 'warning' || kpis.turbidity.status === 'critical';
        return (
          <div
            id="kpi-card-turbidity"
            className={`relative overflow-hidden bg-slate-900/90 border rounded-xl p-4 transition-all duration-200 shadow-sm flex flex-col justify-between ${
              isWarn
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className={`text-[10px] font-medium ${isWarn ? 'text-amber-400' : 'text-emerald-400'}`}>
                    {kpis.turbidity.statusLabel}
                  </div>
                </div>
              </div>
              {isWarn && (
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[10px] text-violet-400 font-medium">{kpis.temperature.statusLabel}</div>
            </div>
          </div>
          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-mono text-slate-400 bg-slate-800/80">
            CELSIUS
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[10px] text-amber-400 font-medium">
                {kpis.activeAlerts.total > 0 ? 'Action Required' : 'All Clear'}
              </div>
            </div>
          </div>
        ...
        <div className="mt-3">
          ...
          <div className="mt-2 text-[11px] text-amber-300/90 font-medium flex items-center gap-1.5">
            ...
            <span id="kpi-alerts-warning-subtext" className="kpi-alert-subtext">
              {kpis.activeAlerts.critical} Critical · {kpis.activeAlerts.warning} Warning · {kpis.activeAlerts.advisory} Advisory
            </span>
          </div>
        </div>
